...
async function login(mac, ip, qstr) {
  let config;
  const {UserName, Password, AidcAuthAttr3, AidcAuthAttr4, AidcAuthAttr5, AidcAuthAttr6, AidcAuthAttr7, AidcAuthAttr8} = qs.parse(
      qstr);

  const baseUrl = '58.53.199.144:8001';
  const agent = superagent.agent();
  let res = await agent.get(baseUrl).set({
    'User-Agent': 'CDMA+WLAN(Maod)',
  }).query({
    'userip': ip,
    'wlanacname': '',
    'nasip': '59.175.245.222',
    'usermac': mac,
    'aidcauthtype': 0,
  });
  ...
  res = await agent.post(config.LoginURL).
      set({
        'User-Agent': 'CDMA+WLAN(Maod)',
